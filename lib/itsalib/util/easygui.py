...
def __choicebox(message, title, choices):
        ...
        import re
        patt = re.compile(r'\d+|[a-zA-Z]')
        def sortkey(s):
            key = []
            for part in patt.findall(s.lower()):
                try:
                    key.append(int(part))
                except ValueError:
                    key.append(ord(part))
            return key
        choices.sort(key=sortkey)
        lastInserted = None
        choiceboxChoices = []
        for choice in choices:
...
# fileopenbox
#-------------------------------------------------------------------
def fileopenbox(msg=None, title=None, argInitialFile=None, filetypes='*', initialdir='/'):
        ...
        root = Tk()
        root.withdraw()
        f = tkFileDialog.askopenfilename(parent=root,title=title, initialfile=argInitialFile, filetypes=filetypes, initialdir=initialdir)
        if f == "": return None
        return f
...
# filesavebox
#-------------------------------------------------------------------
def filesavebox(msg=None, title=None, arginitialdir=None, argInitialFile=None):
        ...
        root = Tk()
        root.withdraw()
        f = tkFileDialog.asksaveasfilename(parent=root, title=title, initialdir=arginitialdir, initialfile=argInitialFile)
        if f == "": return None
        return f
